...
if (isset($_POST['editrecid'])) {
    $emailid = $_POST['editrecid'];

    $datatosend = array("obj" => array("EmailID" => $emailid));
    $posturl = $apiurl . '/SplashService.svc/GetExaminerDetailsByEmailIDWebAPI';

    $responsearray = get_api_response_withdata($posturl, $datatosend);

    $examinerdetailsresponse = $responsearray['GetExaminerDetailsByEmailIDWebAPIResult'];

    $firstname = $examinerdetailsresponse['FirstName'];

    $lastname = $examinerdetailsresponse['LastName'];

    $mobileno = $examinerdetailsresponse['MobileNo'];

    $emailid = $examinerdetailsresponse['ExaminerEmailID'];

    $paswrd = $examinerdetailsresponse['Password'];
...
?>
    <script>
        alert('Invalid examiner to Edit');
...
<div class="main_content">
    ...
    <div id="title_ui" class="col-md-12 pt-4">
        <h3> Examiner(s) >> Editing Examiner</h3>
    </div>
    ...
    <div class="container">
        ...
        <div class="row rowStyle" style="width:100%;text-align:left">
            ...
            <div class="col-md-6">
                ...
                required title="enter valid email address" ; disabled 
        ...
        $.ajax({
            ...
            success: function(dataabc) {
                ...
                if (data['isok'] == 1) {
                    ...
                    $("#firstname_err").html(data['firstname_error_msg'])
                    $("#lastname_err").html(data['lastname_error_msg'])
                    $("#mobileno_err").html(data['mobileno_error_msg'])
                    $("#emailid_err").html(data['emailid_error_msg'])
                    $("#paswrd_err").html(data['paswrd_error_msg'])
                    $("#confirmpasswrd_err").html(data['confirmpasswrd_error_msg'])
