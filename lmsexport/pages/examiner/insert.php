...
$s = $firstnameerror . ' - ' . $lastnameerror . ' - ' . $mobilenoerror . ' - ' . $emailiderror .
' - ' . $paswrderror . ' - ' . $confirmpasswrderror;


if (
    $firstnameerror ||$lastnameerror || $mobilenoerror ||
...
if ($isok) {
    ...
    $datatosend = array("obj" => $formfieldarray);


    $posturl = $apiurl . '/SplashService.svc/CreateExaminerWebAPI';

    $response = get_api_response_withdata($posturl, $datatosend);


    $rv['isok'] = $response['CreateExaminerWebAPIResult']['objStatusCodes'];
    $rv['msg'] = $response['CreateExaminerWebAPIResult']['Message'];
} else {
